...
#
# Real fix for the pill sitting wrong specifically on the Dashboard page
# (About/ProjectView were fine): .dashboard has backdrop-filter, which per
# spec makes it the containing block for its position:fixed descendants --
# so NavPill's bottom:0 was resolving against .dashboard's own box, not the
# real viewport, only there. Portaled NavPill onto document.body so its
# containing block is always the true viewport, no matter what wraps it.
set -e

# Sanity check: fail loudly instead of silently creating stray folders
# if this isn't run from the actual repo root.
for f in src/components/Global/NavPill/index.tsx; do
  if [ ! -f "$f" ]; then
...
cat > src/components/Global/NavPill/index.tsx <<'FILEEOF'
...
echo "Portaled NavPill to document.body -- fixes the Dashboard-only offset."
echo ""
echo "NEXT STEPS:"
echo "1. git add src/components/Global/NavPill/index.tsx"
echo "2. git commit -m 'fix(nav): portal NavPill to document.body, fixing Dashboard-only offset'"
